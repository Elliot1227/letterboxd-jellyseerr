...
Uses letterboxd-rss to fetch the watchlist, checks streaming availability,
...
# ---------------------------------------------------------------------------
# Letterboxd via letterboxd-rss
# ---------------------------------------------------------------------------

def fetch_watchlist() -> list[dict]:
    """
    Run letterboxd-rss to generate feed.xml, then parse it.
    ...
    if result.returncode != 0:
        log.error(f"letterboxd-rss failed:\n{result.stderr}")
        raise RuntimeError("letterboxd-rss failed — check your username and network")
    ...
    for item in channel.findall("item"):
        ...
        # letterboxd-rss puts year in the title like "The Godfather (1972)"
        ...
        if raw_title.endswith(")") and "(" in raw_title:
            title, _, year_part = raw_title.rpartition("(")
            title = title.strip()
...
# ---------------------------------------------------------------------------
# TMDB ID resolution from Letterboxd film page
# ---------------------------------------------------------------------------
...
def sync():
    ...
    for film in films:
        ...
        # Check streaming — use cache, but re-check if empty list (might have changed)
